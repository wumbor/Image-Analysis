//This macro performs measures fluorescence intensity in one channel based on ROIs defined in the other channels of an image stack
...
function AnalyzeDistribution(workingDir) {
	...
	for (i = 0; i < fileList.length; i++){
		if (endsWith(fileList[i], ".tif")) { //process only tif images
			...
			C1ROIs = CreateROIs(C1name, C1color);
